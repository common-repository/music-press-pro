...
$artistID = get_the_ID();
$limit = get_option('options_artist_all_album_limit',8);
$orderby = get_option('options_global_orderby','mp_count_play');
$order = get_option('options_global_order','DESC');

$album_arr = music_press_pro_get_all_albums_from_artist($artistID,$orderby,$order);
if($album_arr ) { ?>
            ...
            while ($the_query->have_posts()): $the_query->the_post();
                ...
                <div class="mp-item">
                    <a class="mp-img" href="<?php echo esc_url(get_permalink(get_the_ID())); ?>"
                       style="background-image:url('<?php echo get_the_post_thumbnail_url(get_the_ID(), 'large'); ?>')"></a>
                    <h3>
