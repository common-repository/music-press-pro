...
if ( ! class_exists( 'Music_Press_Pro_Band_Fields' ) ) {
class Music_Press_Pro_Band_Fields

{
    ...
    public function __construct()
    {
        ...
        add_action('do_meta_boxes', array($this,'music_press_pro_replace_band_featured_image_box'));
        $this->built_in = array(
            'band_banner' => array(
    ...
    public function music_press_pro_replace_band_featured_image_box()
    {
        remove_meta_box( 'postimagediv', 'mp_band', 'side' );
        add_meta_box('postimagediv', __('Band avatar'), 'post_thumbnail_meta_box', 'mp_band', 'side', 'low');
    }
    ...
    public function music_press_pro_filter_posts( $query ) {
        if ( ! is_post_type_archive('mp_band') && ( isset($query->query['post_type']) && 'mp_band' != $query->query['post_type'] )) {
            return;
        }
    ...
    public function music_press_pro_register_fields() {
        ...
        $layout_groups['Description'][] = array (
            'key' => 'field_5band0fe7d4570',
            'label' => esc_html__( 'Band Description',  'music-press-pro' ),
            'name' => 'content',
            'type' => 'wysiwyg',
            'default_value' => '',
            'toolbar' => 'full',
        ...
        $layout_groups['Description'][] = array (
            'key' => 'field_5band0fe7dfd378',
            'label' => esc_html__( 'Band Short description',  'music-press-pro' ),
            'name' => 'band_short_desc',
            'type' => 'textarea',
            'default_value' => '',
            'sort'      => 10,
        );
