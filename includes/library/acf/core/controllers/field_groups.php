...
<div id="acf-about" class="acf-content">
	...
	<div class="acf-content-title">
		...
		<h2><?php esc_html_e("Thank you for updating to the latest version!",'acf'); ?> <br /><?php printf(esc_html__('ACF %s is more polished and enjoyable than ever before. We hope you like it.','acf'),$version); ?></h2>
	</div>
	<!-- / acf-content-title -->
...
<?php if( $tab == 'whats-new' ): 
		...
		$active = array(
			...
			'options_page' => class_exists('acf_options_page_plugin_mp'),
		...
		<?php printf(esc_html__('Migrating your field types is easy, please %sfollow this tutorial%s to learn more.','acf'),'<a href="http://www.advancedcustomfields.com/docs/tutorials/creating-a-new-field-type/" target="_blank">','</a>'); ?></p>
		
		<h4><?php esc_html_e("Actions &amp; Filters",'acf'); ?></h4>
		<p><?php printf(esc_html__('All actions & filters have received a major facelift to make customizing ACF even easier! Please %sread this guide%s to find the updated naming convention.','acf'),'<a href="http://www.advancedcustomfields.com/resources/getting-started/migrating-from-v3-to-v4/" target="_blank">','</a>'); ?></p>
...
<?php elseif( $tab == 'download-add-ons' ): ?>
		...
		<ol>
			...
			<li><?php printf(esc_html__('Navigate to %sPlugins > Add New > Upload%s','acf'),'<a target="_blank" href="'.admin_url('plugin-install.php?tab=upload').'">','</a>'); ?></li>
