...
class acf_addons
{
	...
	*/
	...
	{
		// add page
		$page = add_submenu_page('edit.php?post_type=acf', esc_html__('Add-ons','acf'), esc_html__('Add-ons','acf'), 'manage_options', 'acf-addons', array($this,'html'));
	...
	*/
	...
	function html()
	{
		...
		$premium = array();
		$premium[] = array(
			'title' => esc_html__("Repeater Field",'acf'),
			'description' => esc_html__("Create infinite rows of repeatable data with this versatile interface!",'acf'),
		...
		$premium[] = array(
			'title' => esc_html__("Gallery Field",'acf'),
			'description' => esc_html__("Create image galleries in a simple and intuitive interface!",'acf'),
		...
		$premium[] = array(
			'title' => esc_html__("Options Page",'acf'),
			'description' => esc_html__("Create global data to use throughout your website!",'acf'),
			'thumbnail' => $dir . 'images/add-ons/options-page-thumb.jpg',
			'active' => class_exists('acf_options_page_plugin_mp'),
			'url' => 'http://www.advancedcustomfields.com/add-ons/options-page/'
		);
		$premium[] = array(
			'title' => esc_html__("Flexible Content Field",'acf'),
			'description' => esc_html__("Create unique designs with a flexible content layout manager!",'acf'),
		...
		$free = array();
		$free[] = array(
			'title' => esc_html__("Gravity Forms Field",'acf'),
			'description' => esc_html__("Creates a select field populated with Gravity Forms!",'acf'),
		...
		$free[] = array(
			'title' => esc_html__("Date & Time Picker",'acf'),
			'description' => esc_html__("jQuery date & time picker",'acf'),
		...
		$free[] = array(
			'title' => esc_html__("Google Map Extended",'acf'),
			'description' => esc_html__("Find addresses and coordinates of a desired location",'acf'),
		...
		$free[] = array(
			'title' => esc_html__("Contact Form 7 Field",'acf'),
			'description' => esc_html__("Assign one or more contact form 7 forms to a post",'acf'),
...
<div class="wrap" style="max-width:970px;">
	...
	<h2 style="margin: 4px 0 15px;"><?php esc_html_e("Advanced Custom Fields Add-Ons",'acf'); ?></h2>
	...
	<?php esc_html_e("Each Add-on can be installed as a separate plugin (receives updates) or included in your theme (does not receive updates).",'acf'); ?></p>
	</div>
	<?php /*
	<div class="acf-alert">
		<p><strong><?php esc_html_e("Just updated to version 4?",'acf'); ?></strong> <?php esc_html_e("Activation codes have changed to plugins! Download your purchased add-ons",'acf'); ?> <a href="http://www.advancedcustomfields.com/add-ons-download/" target="_blank"><?php esc_html_e("here",'acf'); ?></a></p>
	</div>
	*/ ?>
	...
	<div id="add-ons" class="clearfix">
		...
		<div class="add-on wp-box <?php if( $addon['active'] ): ?>add-on-active<?php endif; ?>">
			...
			<div class="footer">
				...
				<?php else: ?>
					<a target="_blank" href="<?php echo $addon['url']; ?>" class="button"><?php esc_html_e("Purchase & Install",'acf'); ?></a>
				<?php endif; ?>
			</div>
		</div>
		<?php endforeach; ?>
		...
		<div class="add-on wp-box <?php if( $addon['active'] ): ?>add-on-active<?php endif; ?>">
			...
			<div class="footer">
				<?php if( $addon['active'] ): ?>
					<a class="button button-disabled"><span class="acf-sprite-tick"></span><?php esc_html_e("Installed",'acf'); ?></a>
				<?php else: ?>
					<a target="_blank" href="<?php echo $addon['url']; ?>" class="button"><?php esc_html_e("Download",'acf'); ?></a>
				<?php endif; ?>
			</div>
		</div>
		<?php endforeach; ?>	
