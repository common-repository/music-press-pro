...
class acf_options_page_plugin_mp
{
	var $settings;
	...
	*/
	...
	function __construct()
	{
		// vars
		$this->settings = array(
			'title' 		=> esc_html__('Options','acf'),
			'menu'			=> esc_html__('Options','acf'),
			'slug' 			=> 'acf-options',
	...
	*/
	...
	{
	    $choices[ esc_html__("Options Page",'acf') ]['options_page'] = esc_html__("Options Page",'acf');
	...
	*/
	...
	function admin_head()
	{
		...
		if( isset($_POST['acf_nonce']) && wp_verify_nonce($_POST['acf_nonce'], 'input') )
		{
			...
			$this->data['admin_message'] = esc_html__("Options Updated",'acf');
		}
	...
	*/
	...
	function html()
	{
		...
		<div class="wrap no_move">
			...
			<div id="message" class="updated"><p><?php esc_html_e("No Custom Field Group found for the options page",'acf'); ?>. <a href="<?php echo admin_url(); ?>post-new.php?post_type=acf"><?php esc_html_e("Create a Custom Field Group",'acf'); ?></a></p></div>
			<?php else: ?>
			...
			<div class="metabox-holder has-right-sidebar" id="poststuff">
				...
				<div class="inner-sidebar" id="side-info-column">
					...
					<!-- Update -->
					<div class="postbox">
						<h3 class="hndle"><span><?php esc_html_e("Publish",'acf'); ?></span></h3>
						<div class="inside">
							...
							<input type="submit" class="acf-button" value="<?php esc_html_e("Save Options",'acf'); ?>" />
						</div>
					</div>

					<?php $meta_boxes = do_meta_boxes('acf_options_page_mp', 'side', null); ?>
				...
				<div id="post-body-content">
					<?php $meta_boxes = do_meta_boxes('acf_options_page_mp', 'normal', null); ?>
					<script type="text/javascript">
					(function($){
...
$GLOBALS['acf_options_page_mp'] = new acf_options_page_plugin_mp();
...
{
	$GLOBALS['acf_options_page_mp']->settings['title'] = $title;
}
...
{
	$GLOBALS['acf_options_page_mp']->settings['menu'] = $menu;
}
...
{
	$GLOBALS['acf_options_page_mp']->settings['capability'] = $capability;
}
...
function acf_add_options_sub_page( $page = false )
{
	...
	$GLOBALS['acf_options_page_mp']->settings['pages'][] = $page;
}
